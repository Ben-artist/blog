...
title: 深入浅出事件循环
date: 2025-03-03
description: 从 JavaScript 单线程模型出发，讲解事件循环、宏任务与微任务的执行顺序，以及浏览器中的调度机制。
tags: ['JavaScript', '事件循环', '异步编程', '浏览器']
---
JavaScript 是单线程语言，却需要同时处理用户交互、网络请求与定时任务。事件循环（Event Loop）就是浏览器协调这些工作的核心机制。本文从「为什么是单线程」讲起，帮你建立清晰的心智模型。
